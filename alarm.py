...
# load the model
modelPath = "C:/Users/Daryl Guerzon/Desktop/models/train5/etian-last5.pt"
...
pygame.mixer.init()
sound_path = "C:/Users/Daryl Guerzon/Desktop/try sound/try sound.mp3"  # Add the path to the sound file
chicken_sound = pygame.mixer.Sound(sound_path)
    ...
    while cap.isOpened():
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                cv2.circle(im0, (centerX, centerY), 5, (0, 255, 0),
                           -1)  # put circles in the centers of each bounding box
                cv2.putText(im0, f"{names[cls]} {conf:.2f}", (int(x1), int(y1) - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255),
                            2)  # print the detected class name and confidence percentage
                ...
                for j in range(len(centers)):
                    if i != j:  # don't compare a chicken to itself
                        ...
                        if dist < distanceTreshold:
                            ...
                            cv2.line(im0, centers[i], centers[j], (0, 0, 255),
                                     2)  # draws the line between chickens that are close to each other
            ...
            for idx, (center, bbox, isolated) in enumerate(zip(centers, boundingBoxes, isolatedFlags)):
                if isolated:
                    cv2.putText(im0, "Isolated", (int(bbox[0]), int(bbox[1]) - 40),
                                ...
                                2)  # prints isolation mark on top of the bounding boxes
                    cv2.rectangle(im0, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (0, 0, 255),
                                  2)  # draws red bounding boxes on isolated chickens

                    # Play sound if a chicken is isolated
                    chicken_sound.play()  # Sound will play when a chicken is marked isolated
